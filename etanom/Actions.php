...
Class Actions extends DBConnection{
    ...
    function save_category(){
        ...
        if(@$check> 0){
            $resp['status'] ='failed';
            $resp['msg'] = 'crops Category already exists.';
        }else{
            @$save = $this->query($sql);
            if($save){
                $resp['status']="success";
                if(empty($id))
                    $resp['msg'] = "crops Category successfully saved.";
                else
                    $resp['msg'] = "crops Category successfully updated.";
            }else{
                $resp['status']="failed";
                if(empty($id))
                    $resp['msg'] = "Saving New crops Category Failed.";
                else
                    $resp['msg'] = "Updating crops Category Failed.";
                $resp['error']=$this->lastErrorMsg();
                $resp['sql']=$sql;
            }
        }
    ...
    function save_crops(){
        ...
        foreach($_POST as $k => $v){
            if(in_array($k,array('Planning','Land_Preparation','Planting_Stages','other_info')))
            $v = htmlentities($v);
            if(!in_array($k,array('id'))){
                $v = trim($v);
                $v = $this->escapeString($v);
        ...
        if(empty($id)){
            $sql = "INSERT INTO `crops_list` ({$cols_join}) VALUES ($vals_join)";
        }else{
            $sql = "UPDATE `crops_list` set {$data} where crops_id = '{$id}'";
        }
        ...
        if($save){
            $resp['status']="success";
            if(empty($id))
                $resp['msg'] = "crops successfully saved.";
            else
                $resp['msg'] = "crops successfully updated.";
            if(empty($id)){
                $crops_id = $this->query("SELECT last_insert_rowid()")->fetchArray()[0];
            }else{
                $crops_id = $id;
            }
            $dir = __DIR__.'/uploads/';
            ...
            if(isset($_FILES['image']) && !empty($_FILES['image']['tmp_name'])){
                ...
                if(!in_array($type,array('image/png','image/jpeg'))){
                    $resp['msg'] = "crops successfully saved but image upload failed due to invalid file type";
                }else{
                    $gdImage = ($type == 'image/png') ? imagecreatefrompng($upload) :   imagecreatefromjpeg($upload);
                    if($gdImage){
                        $filePath = $dir.$crops_id.'.png';
                        if(is_file($filePath))
                        unlink($filePath);
                        imagepng($gdImage,$filePath);
                        imagedestroy($gdImage);
                    }else{
                            $resp['msg'] = "crops successfully saved but image upload failed due to unknown reason";
                    }
                }
            ...
            $resp['status']="failed";
            if(empty($id))
                $resp['msg'] = "Saving New crops Failed.";
            else
                $resp['msg'] = "Updating crops Failed.";
            $resp['error']=$this->lastErrorMsg();
        }
        return json_encode($resp);
    }
    function delete_crops(){
        ...
        @$delete = $this->query("DELETE FROM `crops_list` where crops_id = '{$id}'");
        if($delete){
            $resp['status']='success';
            $_SESSION['flashdata']['type'] = 'success';
            $_SESSION['flashdata']['msg'] = ' successfully deleted.';
...
switch($a){
    ...
    break;
    case 'save_crops':
        echo $action->save_crops();
    break;
    case 'delete_crops':
        echo $action->delete_crops();
    break;
    case 'save_comment':
